...
$currentPic = htmlspecialchars(basename($user['profile_pic'] ?? 'admin1.png'));
?>
...
<form id="profileForm" method="POST" action="update_profile.php" class="p-3 border rounded bg-white">
  <div class="text-center mb-3">
    <img id="previewPic" src="avatars/<?= 'admins/' . $currentPic ?>" alt="Profile Picture" style="width:100px; height:100px; border-radius:50%; object-fit:cover; border:3px solid #1a7c4a;">
  </div>
  ...
  <div class="mb-3">
    <label class="form-label">Choose an Admin Avatar</label>
    <div class="d-flex flex-wrap gap-3">
      ...
      ?>
        <label style="cursor:pointer;">
          ...
          <img src="avatars/admins/<?= $fn ?>" onclick="selectAvatar('admins/<?= $fn ?>','<?= $fn ?>')" style="width:70px; height:70px; border-radius:50%; border:3px solid <?= $checked ? '#1a7c4a' : '#ccc' ?>; object-fit:cover;">
        </label>
      <?php endfor; ?>
    </div>
  </div>
